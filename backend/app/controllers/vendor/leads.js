...
export const getLeads = (req, res) => {
    ...
    try {
        ...
        const query =  "SELECT leads.*, users.name AS member_name, users.email AS member_email, users.profile_img AS member_image FROM  leads LEFT JOIN  users  ON  leads.user_id = users.id WHERE leads.vendor_id = ?";
        executeQuery({
            query,
            data: [user_id],
